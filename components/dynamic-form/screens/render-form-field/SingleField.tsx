...
import {
  CreditCard,
  CreditCardValue,
} from "@/components/ui/extension/credit-card";
...
import {
  ...
  CommandItem,
  CommandList,
} from "web-utils-components/command";
import {
  FormControl,
  FormDescription,
  FormField,
  ...
  FormLabel,
  FormMessage,
} from "web-utils-components/form";
...
import LocationSelector from "@/components/ui/extension/location-input";
...
import { SignatureMaker } from "@/components/ui/extension/signature-maker";
...
import { getNodesByIds } from "../../utils/treeOps";
              ...
              <FormControl>
                <Dialog open={openTreeDialog} onOpenChange={closeTreeDialog}>
                  <DialogTrigger>
                    <p
                      className={cn(
                        "w-full rounded-md border border-input bg-background px-3 py-2 text-sm shadow-sm  text-start select-text cursor-text",
                        field.className,
                        formField.value
                          ? "text-foreground"
                          : "text-muted-foreground",
                        (field.disabled || condDisabled) &&
                          "opacity-50 cursor-not-allowed",
                        formField.value.length === 0 && "text-muted-foreground"
                      )}
                    >
                  ...
                  <DialogContent className="!max-w-3xl w-full min-h-64 p-6 pt-7 bg-white dark:bg-gray-900 rounded-lg shadow-lg border border-gray-200 dark:border-gray-800">
                    <Tree
                      data={field.tree}
                      selected={new Set(formField.value.split(", "))}
                      parentSelection={field.treeSettings?.parentSelection}
                      treeLines={field.treeSettings?.treeLines}
                      multiple={field.treeSettings?.multiple}
                      selection={field.treeSettings?.selectable}
                      onSelect={(selected) => {
                        setTimeout(() => {
                          formField.onChange(
                            Array.from(selected)
