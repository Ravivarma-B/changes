...
type Doc = Document<{ id: string; name: string }>;
...
type CellData = {
  ...
  value?: string;
  onChange: (icon: string, isUserIcon: boolean) => void;
  search?: string;
};
...
const Cell = React.memo(
  ({
    ...
    return (
      <div style={style} className="flex items-center justify-center !h-auto">
        <Button
          variant="ghost"
          ...
          onClick={() => onChange(iconName, typeof icon !== "string")}
          title={
...
export function IconPicker({
  ...
  const indexRef = useRef<Doc | null>(null);

  const fetchUserIcons = useCallback(async () => {
    try {
      setIsLoading(true);
      const res = await fetch(`/admin/api/upload-icon`);
      setIsLoading(false);
      if (!res.ok) return;
  ...
  useMemo(() => {
    const idx: Doc = new Document<{ id: string; name: string }>({
  ...
  const filteredIcons = useMemo(() => {
    ...
    const results: ReturnType<Doc["search"]> =
      indexRef.current?.search(search) ?? [];

    const names = results.flatMap((r) => r.result).map(String);
    return names.map((name) => iconMap.get(name)!);
  }, [search, iconMap]);
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="border rounded-md !scrollbar-hide h-[250px]">
        ...
        {isLoading && <SpinnerCircular className="!w-4 h-4 m-1" />}
      </div>
    </div>
  );
}
