              ...
              <FormControl>
                <Dialog open={openTreeDialog} onOpenChange={closeTreeDialog}>
                  ...
                  <DialogContent className="!max-w-3xl w-full min-h-64 p-6 pt-7 bg-white dark:bg-gray-900 rounded-lg shadow-lg border border-gray-200 dark:border-gray-800">
                    ...
                    <DialogTitle className="sr-only">Edit Field:</DialogTitle>
                    <Tree
                      data={field.tree}
                      disableSelection={
                        !(field.treeSettings.selectable ?? true)
                      }
                      showConnectedLines={field.treeSettings.treeLines ?? false}
                      multiple={field.treeSettings.multiple ?? false}
                      viewOnly={!(field.treeSettings.editable ?? false)}
                      onSelect={(selected) => {
                        form.setValue(
                          field.name,
                          Array.from(selected).join(", ").toString(),
                          {
                            shouldValidate: true,
                            shouldDirty: true,
                          }
